...
form.addEventListener("submit",function(e){
    ...
    const isRequireValid=checkRequire([username,email,password,confirmPassword]);

    let isFormValid=isRequireValid;
    if(isRequireValid){
...
function checkRequire(inputArray){
    ...
    inputArray.forEach((input) => {
        if(input.value.trim()===""){
            showError(input,`${formatFieldName(input)} is require`)
            isValid=false
        }else{
            showSuccess(input);
        }
